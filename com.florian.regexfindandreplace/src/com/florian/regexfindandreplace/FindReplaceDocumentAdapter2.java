...
public class FindReplaceDocumentAdapter2 extends FindReplaceDocumentAdapter {
	...
	/**
	 * Returns the location of a given string in this adapter's document based
	 ...
	 *            spaces as defined by Character.isWhiteSpace. Must not be used
	 *            in combination with <code>regExSearch</code>.
	 * @param regExSearch
	 *            if <code>true</code> findString represents a regular
	 *            expression Must not be used in combination with
	 *            <code>wholeWord</code>.
	 * @return the find or replace region or <code>null</code> if there was no
	 *         match
	...
	/**
	 * Stateful findReplace executes a FIND, REPLACE, REPLACE_FIND or FIND_FIRST
	 * operation. In case of REPLACE and REPLACE_FIND it sends a
	 * <code>DocumentEvent</code> to all registered
	 * <code>IDocumentListener</code>.
	 *
	 * @param startOffset
	 *            document offset at which search starts this value is only used
	 *            in the FIND_FIRST operation and otherwise ignored
	 * @param findString
	 *            the string to find this value is only used in the FIND_FIRST
	 *            operation and otherwise ignored
	 * @param replaceText
	 *            the string to replace the current match this value is only
	 *            used in the REPLACE and REPLACE_FIND operations and otherwise
	 *            ignored
	 * @param forwardSearch
	 *            the search direction
	 * @param caseSensitive
	 ...
	 *            in combination with <code>regExSearch</code>.
	 * @param regExSearch
	 *            if <code>true</code> this operation represents a regular
	 *            expression Must not be used in combination with
	 *            <code>wholeWord</code>.
	 * @param operationCode
	 ...
	 *             if a REPLACE or REPLACE_FIND operation is not preceded by a
	...
	private IRegion findReplace(final FindReplaceOperationCode operationCode, int startOffset, String findString,
		...
		if (operationCode == FIND_FIRST) {
			...
			if (fFindReplaceMatcher != null && fFindReplaceMatcher.pattern().pattern().equals(findString)
					&& fFindReplaceMatcher.pattern().flags() == patternFlags) {
				/*
				 * Commented out for optimization: The call is not needed since
				 * FIND_FIRST uses find(int) which resets the matcher
				 */
		...
		if (operationCode == REPLACE || operationCode == REPLACE_FIND_NEXT) {
			...
			if (fDocument instanceof IRepairableDocumentExtension && ((IRepairableDocumentExtension) fDocument)
					.isLineInformationRepairNeeded(offset, length, replaceText)) {
				String message = "FindReplaceDocumentAdapter.incompatibleLineDelimiter"; //$NON-NLS-1$
		...
		if (operationCode != REPLACE) {
			try {
				...
				int index = -1;
				int length = -1;
				while (found && fFindReplaceMatcher.start()
						+ fFindReplaceMatcher.group().length() <= fFindReplaceMatchOffset + 1) {
				...
				String message = "FindReplaceDocumentAdapter.patternTooComplex"; //$NON-NLS-1$
				...
				if (i + 1 < length) {
					...
					if (inQuote) {
						...
						if (inCharGroup > 0 || inBraces > 0) {
							String msg = "FindReplaceDocumentAdapter.illegalLinebreak"; //$NON-NLS-1$
	...
	private String interpretReplaceEscapes(String replaceText, String foundText) {
		...
		for (int i = 0; i < length; i++) {
			...
			if (inEscape) {
				...
				/*
				 * Feature in java.util.regex.Matcher#replaceFirst(String): $00,
				 * $000, etc. are interpreted as $0 and $01, $001, etc. are
				 * interpreted as $1, etc. . If we support \0 as replacement
				 * pattern for capturing group 0, it would not be possible any
				 * more to write a replacement pattern that appends 0 to a
				 * capturing group (like $0\0). The fix is to interpret \00 and
				 * $00 as $0\0, and \01 and $01 as $0\1, etc.
				 */
				if (i + 2 < length) {
		...
		if (inEscape) {
			// '\' as last character is invalid, but we still add it to get an
			// error message
			buf.append('\\');
		}
		return buf.toString();
	 ...
	 * of the <code>replaceText</code> and appends the interpretation to
	 * <code>buf</code>.
	 *
	 * @param ch
	...
	private int interpretReplaceEscape(final char ch, int i, StringBuffer buf, String replaceText, String foundText) {
		...
		case 'c':
			if (i + 1 < length) {
				...
				String msg = "FindReplaceDocumentAdapter.illegalControlEscape" + "  " + "\\c"; //$NON-NLS-1$ //$NON-NLS-2$
		...
		case 'x':
			if (i + 2 < length) {
				...
				try {
					...
					String msg = "FindReplaceDocumentAdapter.illegalHexEscape" + "  "
		...
		case 'u':
			if (i + 4 < length) {
				...
				try {
					...
					String msg = "FindReplaceDocumentAdapter.illegalUnicodeEscape" + "  "
	...
	/**
	 * Substitutes the previous match with the given text. Sends a
	 * <code>DocumentEvent</code> to all registered
	 * <code>IDocumentListener</code>.
	 *
	 * @param text
	 ...
	 * @param regExReplace
	 *            if <code>true</code> <code>text</code> represents a regular
	 *            expression
